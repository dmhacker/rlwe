...
TEST_CASE("Experimental homomorphic key exchange") {
  ...
  NTL::ZZX a_shared = rlwe::UniformSample(params.GetPolyModulusDegree(), ZZ(0), params.GetCoeffModulus()); 

  // Alice keeps these parameters private
  NTL::ZZX e_alice = rlwe::GaussianSample(params.GetPolyModulusDegree(), params.GetErrorStandardDeviation());
  ...
  rlwe::Ciphertext s_alice_encrypted_alice = hp_alice.Encrypt(rlwe::Plaintext(s_alice.GetSecret(), leveled_params));

  // Bob keeps these parameters private
  NTL::ZZX e_bob = rlwe::GaussianSample(params.GetPolyModulusDegree(), params.GetErrorStandardDeviation());
  ...
  rlwe::Ciphertext s_bob_encrypted_bob = hp_bob.Encrypt(rlwe::Plaintext(s_bob.GetSecret(), leveled_params));

  // Alice calculates this privately and then publishes the result
  rlwe::Ciphertext key_bob_encrypted_bob = 
    (-hp_bob.Encrypt(rlwe::Plaintext(p_bob.GetValues().a, leveled_params)) * hp_bob.Encrypt(rlwe::Plaintext(s_alice.GetSecret(), leveled_params))) + 
  ...
  rlwe::Ciphertext key_alice_encrypted_alice =
    (-hp_alice.Encrypt(rlwe::Plaintext(p_alice.GetValues().a, leveled_params)) * hp_alice.Encrypt(rlwe::Plaintext(s_bob.GetSecret(), leveled_params))) + 
